...
$('li').unbind("mousedown").mousedown(function (e) {
    ...
    if (count === 0) {
        count = 1;
        var doc_x = this.offsetLeft;
        var doc_y = this.offsetTop;
    }
    doc = this;
    doc1 = this.innerHTML;
    $(document).mousemove(function (e) {
        ...
        window.a = e.pageX - x + display_area1.offsetLeft - 150;
        window.b = e.pageY - y + display_area1.offsetTop + 100;

    })
    $(document).off('mouseup');
    $(document).on('mouseup', function () {
        if (doc1 === $('.lis1').html() || doc1 === $('.lis3').html() || doc1 === $('.lis12').html() || doc1 === $('.lis14').html() || doc1 === $('.lis18').html()) {
            if (window.a >= x1 && window.a <= x1 + 300 && window.b >= y1 && window.b <= y1 + 370) {
                window.a = 0;
                $('.true p').show();
                $('.true p').delay(2000).hide(0);
                doc.style.display = 'none';
                count = 0;
            } else if (window.a >= x2 && window.a <= x2 + 264 && window.b >= y2 && window.b <= y2 + 370) {
                count = 0;
                $('.false p').show();
                $('.false p').delay(2000).hide(0);
                doc.style.left = doc_x + 'px';
                doc.style.top = doc_y + 'px';
                window.a = 0;
            } else if (window.a >= x3 && window.a <= x3 + 270 && window.b >= y3 && window.b <= y3 + 370) {
                count = 0;
                doc.style.left = doc_x + 'px';
                doc.style.top = doc_y + 'px';
                $('.false p').show();
        ...
        if ( doc1 === $('.lis5').html() || doc1 === $('.lis7').html() || doc1 === $('.lis9').html() || doc1 === $('.lis10').html() || doc1 === $('.lis15').html()) {
            if (window.a >= x1 && window.a <= x1 + 300 && window.b >= y1-100 && window.b <= y1 + 370) {
                window.a = 0;
                count = 0;
                doc.style.left = doc_x + 'px';
                doc.style.top = doc_y + 'px';
                $('.false p').show();
                $('.false p').delay(2000).hide(0);         
            } else if (window.a >= x2 && window.a <= x2 + 264 && window.b >= y2-100 && window.b <= y2 + 370) {
                count = 0;
                window.a = 0;
                doc.style.left = doc_x + 'px';
                doc.style.top = doc_y + 'px';
                $('.false p').show();
                $('.false p').delay(2000).hide(0);          
            } else if (window.a >= x3 && window.a <= x3 + 270 && window.b >= y3-300 && window.b <= y3 + 370) {
                count = 0;
                window.a = 0;
                doc.style.display = 'none';
                $('.true p').show();
